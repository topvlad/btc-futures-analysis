# streamlit_app.py — v1.10
# BTC/Top-N dashboard: dynamic universe (CoinGecko → Binance USDT-perps), regime, RVs, Plan A/B, chart overlays
# Includes: safe auto-refresh (closed bars only), generic OKX fallback, per-coin news term.
...
# Common coin name aliases for nicer news matching
COIN_NAME = {
...
def http_json(url: str, params=None, timeout=8, allow_worker=False):
    ...
    for label, full, p in attempts:
        try:
            r = requests.get(full, params=None if label=="WORKER" else p, timeout=timeout, headers={"Accept":"application/json","User-Agent":"binfapp/1.10"})
...
def http_text(url: str, timeout=8):
    r = requests.get(url, timeout=timeout, headers={"User-Agent":"binfapp/1.10"}); r.raise_for_status()
    return r.text
...
def _discover_top_coins(top_n: int = TOPN_DEFAULT):
    ...
    try:
        j = requests.get(
            ...
            timeout=8, headers={"Accept":"application/json","User-Agent":"binfapp/1.10"}
        ).json()
        bases = []
        for it in j:
...
@st.cache_data(ttl=3600, show_spinner=False)
def _binance_futures_universe(candidates: list[str]):
...
# Universe controls
topn = st.sidebar.number_input("Universe size (Top-N by market cap)", min_value=5, max_value=30, value=TOPN_DEFAULT, step=1, help="Coins are sourced from CoinGecko top market caps, then filtered to Binance USDT perps.")
...
# Build the dynamic universe
bases = _discover_top_coins(topn + (st.session_state.universe_seed and 0))  # seed not used, only to allow manual refresh via next call
SYMBOLS = _binance_futures_universe(bases + (st.session_state.universe_seed and []))
# Ensure BTC first if available
...
# Force client reload + cache seed
refresh_seed = None
if auto_refresh:
...
try:
    tf_data = get_all_tf_data(symbol, TFS, limit_each=1000, _seed=(int(time.time()//60) if auto_refresh else None))
except Exception as e:
...
fdf, fsrc = fetch_funding(symbol, limit=48, _seed=(int(time.time()//60) if auto_refresh else None))
odf, osrc = fetch_open_interest(symbol, _seed=(int(time.time()//60) if auto_refresh else None))
...
news_items = fetch_news(NEWS_FEEDS, NEWS_LOOKBACK_HOURS, 8, term=_news_term(symbol), _seed=(int(time.time()//60) if auto_refresh else None))
...
if report_url:
    try: payload = fetch_report(report_url, _seed=(int(time.time()//60) if auto_refresh else None))
